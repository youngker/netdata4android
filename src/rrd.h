...
#define rrdset_duration(st) ((long long)( (((st)->counter >= ((unsigned long)(st)->entries))?(unsigned long)(st)->entries:(st)->counter) * (st)->update_every))

// get the timestamp of the last entry in the round robin database
#define rrdset_last_entry_t(st) ((long long)((((st)->last_updated.tv_sec) * 1000000ULL) + ((st)->last_updated.tv_usec)))

// get the timestamp of first entry in the round robin database
#define rrdset_first_entry_t(st) ((long long)(rrdset_last_entry_t(st) - rrdset_duration(st)))
...
#define rrdset_time2slot(st, t) ( \
        ...
        ( (rrdset_last_slot(st) >= (unsigned long long)((rrdset_last_entry_t(st) - (t)) / (unsigned long long)((st)->update_every)) ) ? \
          (rrdset_last_slot(st) -  (unsigned long long)((rrdset_last_entry_t(st) - (t)) / (unsigned long long)((st)->update_every)) ) : \
          (rrdset_last_slot(st) -  (unsigned long long)((rrdset_last_entry_t(st) - (t)) / (unsigned long long)((st)->update_every)) + (unsigned long long)(st)->entries ) \
        )))

// get the timestamp of a specific slot in the round robin database
#define rrdset_slot2time(st, slot) ( rrdset_last_entry_t(st) - \
        ((long long)(st)->update_every * ( \
